...
| Workflow-state authority should reside in Nexus whenever deterministic evidence exists — specifically, that backend-determined classification of `context-insufficient` is at least as correct and reliable as the current agent-text approach, without introducing new false positives | Architecture Review (between Iteration 8 and Iteration 9) | The claim is architecturally well-reasoned (a direct extension of R-1's "agents report, they do not write," now stated explicitly — see `docs/MVP_ARCHITECTURE_V2.md` R-1) but has never been implemented or run. No evidence yet distinguishes "correctly deterministic" from "over-eager" — see the paired Open Question, below. | Iteration 9 (`docs/history/iteration-9/SCOPE.md`): implement the minimal backend classifier, re-run Iteration 8's own validated scenario to confirm no regression, and construct a scenario where a refusal is genuinely irrelevant to task completion to see whether backend-only classification over-triggers relative to the agent-narrated approach. |
...
5. **Can deterministic backend classification of `RunBlocked` over-trigger
   compared to agent-reported blockage?** Raised by the Architecture
   Review between Iterations 8 and 9, prompted by the observation that
   Nexus already has the data to classify `context-insufficient` itself
   (see Validated, above) but has never done so — only ever trusted the
   agent's own text. No evidence exists either way: both of Iteration 8's
   real runs involved a refusal that was genuinely relevant to task
   completion, by design (`docs/history/iteration-8/SCOPE.md` §5.A); no
   run has tested a refusal the agent successfully worked around. See
   Unproven, above, and `docs/history/iteration-9/SCOPE.md`.
