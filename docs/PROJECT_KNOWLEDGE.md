...
| A backend-derived signal exists that correctly distinguishes a task-blocking refusal from one the agent legitimately worked around, without reintroducing reliance on agent-authored text | Iteration 9 | Iteration 9 deliberately did not attempt to design one — evidence-before-redesign. Two directions are named (correlate a refusal with the agent's own stated need for it, which reintroduces text-parsing; scope refusals to only the Work Package's own declared `components`/`capabilities`) but neither is evaluated or chosen. | Implement one candidate signal and re-run both of Iteration 9's own scenarios (the relevant refusal and the irrelevant one) to check whether it correctly classifies both — `RunBlocked` for the first, `RunCompleted` for the second — which neither the pure agent-text approach nor Iteration 9's own naive backend rule achieves simultaneously today. |
...
5. **What backend-derived signal correctly distinguishes a task-blocking
   refusal from one the agent legitimately worked around?** Narrowed
   from "can deterministic backend classification over-trigger" (raised
   by the Architecture Review between Iterations 8 and 9) — Iteration 9
   answered that question directly: yes, confirmed by a real run where a
   genuinely irrelevant, explicitly optional refusal was still classified
