...
7. **Does citing an existing `architecture.decision` row give Technology
   ...
   Raised during Iteration 15 scoping, not resolved by it: Iteration 15
   requires a `technology_profile` row to cite an existing, `accepted`
   Decision, but `architecture.decision` itself has no governed creation
   path — every row is a direct-inserted YAML seed, the same gap
   Iteration 12 disclosed and left open for Decisions when it closed the
   equivalent gap for Elements via `provide`
   (`docs/history/iteration-12/LESSONS.md`). Iteration 15's own evidence
   will show only that the citation check works against a pre-seeded
   Decision, not a real "Product Owner proposes a new stack decision"
   workflow. Revisit once a real governed Decision-creation path is
   needed for any reason, not only for Technology Profiles — see
   `docs/history/iteration-15/SCOPE.md`.
